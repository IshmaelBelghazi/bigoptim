...
sim_data <- .simulate_gaussian(true_params, sample_size, intercept=FALSE)
## Empirical data
data(covtype.libsvm)
## Test parmeters
eps <- 1e-02
...
sag_sim_fits <- lapply(SAG, function(alg) alg(sim_data$X,
                                              ...
                                              family=family))
## Fitting with glmnet
glm_fit <- glmnet(sim_data$X, sim_data$y, family="gaussian", intercept=FALSE,
                  lambda=0)
...
test_that("SAG and glmnet results are consistent on simulated data", {
  expect_less_than(norm(sag_sim_fits$constant$w - glmnet_sim_hat, 'F'), 0.01)
  expect_less_than(norm(sag_sim_fits$linesearch$w - glmnet_sim_hat, 'F'), 0.01)
})
